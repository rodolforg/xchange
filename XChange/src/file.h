 ...
 * @return a newly allocated handler. Should be freed by xchange_close(). Returns NULL on failure.
 */
XChangeFile * xchange_open(const char *path, const char *mode);

/**
 * Save file previously opened by xchange_open() with its changes.
 ...
 * This undone modification can be redone by xchange_redo() if no other modification is done meanwhile.
 *
 * In order to any action can be undone, history must be enabled by xchange_enable_history().
...
/**
 * Redo the last undone file modification by xchange_undo().
 * It should be clear this does not repeat the last file modification.
 *
 * In order to any action can be redone, history must be enabled by xchange_enable_history().
...
/**
 * Check if there is any action to be undone by using xchange_undo().
 *
 * @param[in] xfile The file handler.
...
/**
 * Check if there is any action to be redone by using xchange_redo().
 *
 * @param[in] xfile The file handler.
...
/**
 * Get the number of actions that can be undone by using xchange_undo().
 *
 * @param[in] xfile The file handler.
...
/**
 * Get the number of actions that can be redone by using xchange_redo().
 *
 * @param[in] xfile The file handler.
 ...
 * Later modifications will be recorded to be undone by xchange_undo() if
 * history is still enabled by xchange_enable_history().
 *
 * This function does not disable file modification history. This should be
 * done via xchange_enable_history().
 *
 * @param[in] xfile The file handler.
 */
 ...
 * You can undo it by using xchange_undo().
 *
 * Disabling history clear it. See xchange_clear_history().
 *
 ...
 *
 * If it fails, xchange_save() cannot be performed because it is unsafe.
 * However, xchange_save_as() and xchange_save_copy_as() can be executed,
