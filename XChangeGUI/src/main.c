...
static XChangeTable *carregar_tabela(const char *filename, const char *encoding)
{
	...
	xchange_table_set_byte_escape_patternUTF8(xt, preferencias.prefixo_byte_cru, -1, preferencias.sufixo_byte_cru, -1);
...
static void altera_padrao_codificacao_caracteres(char * encoding, gpointer data)
{
	...
	if (xt_new != NULL)
	{
		...
		xchange_table_set_byte_escape_patternUTF8(xt_new, preferencias.prefixo_byte_cru, -1, preferencias.sufixo_byte_cru, -1);
...
static void mudou_conteudo(XChangeHexView *hexv, gpointer data)
{
	...
	if ( (xchange_file_is_history_enabled(xf) && changes_until_last_save == xchange_file_get_undo_list_size(xf))
			|| xchange_hex_view_get_file(XCHANGE_HEX_VIEW(hexv)) == NULL)
	{
		gtk_label_set_text(GTK_LABEL(label_arquivo_modificado), " ");
